...
class TraitCalculator:
    ...
    def add_ts(self, ts):
        ...
        self.sexes = self.ts.tables.individuals.metadata_vector("sex")
        self.num_traits = len(self.ts_metadata["SLiM"]["traits"])
        self.types = [m["type"] for m in self.ts_metadata["SLiM"]["traits"]]
        self.accumulates = [
            m["substitutionAccumulation"] for m in self.ts_metadata["SLiM"]["traits"]
        ]
        # to properly do traits in the case where substitutionAccumulation=F
        # and there are mutation types with convertToSubstitution=T then we
        # need to store this in metadata: (so, this code won't work if this is
        # not present for such traits, which are probably only the default trait!)
        if (
            ...
            # guess T if there just a single trait without substitution accumulation
            ...
            convert = (
                ...
                and self.ts_metadata["SLiM"]["traits"][0]["substitutionAccumulation"]
                == False
            )
    ...
    def get_frequencies(self):
        ...
        # and substitution accumulation is off, so we need to look up both these things:
    ...
    def do_offsets(self):
        ...
        for k, t in enumerate(self.types):
            bO = np.array(
                [self.ts_metadata["SLiM"]["traits"][k][okeys[s]] for s in self.sexes]
            )
            if t == "multiplicative":
    ...
    def multiplicative_effect(self, ind, trait_id, hemizygous):
        ...
        for v in self.ts.variants(samples=ind.nodes, isolated_as_missing=False):
            ...
            for m in muts:
                ...
                # negative effects are "clamped" to zero
                out *= max(0, 1 + h * s)
        return out
