...
def run_slim(recipe, out_dir, recipe_dir="test_recipes", species=None, **kwargs):
    ...
    print(
        f"Running {' '.join(cmd)}, outputting errors etc. to '{out_dir}/SLiM_run_output.log'"
    )
    ...
    if retval != 0:
        ...
        errline = False
        with open(os.path.join(out_dir, "SLiM_run_output.log")) as err:
            for line in err:
...
def _dict_to_eidos(d):
    txt = (
        "Dictionary("
        + ", ".join([f'"{a}", {_obj_to_eidos(b)}' for a, b in d.items()])
        + ")"
    )
    return txt
...
class HelperFunctions:
    @classmethod
    def run_slim_restart(
        cls, in_ts, recipe, out_dir, multichrom, subpop_map=None, **kwargs
    ):
        ...
        for n in [2, 20]:
            for recrate in [0.0, 0.01]:
                ...
                demography.add_population_split(
                    time=5, derived=["A", "B"], ancestral="C"
                )
                ts = msprime.sim_ancestry(
                    {"A": n, "B": n},
