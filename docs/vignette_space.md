...
to {func}`.individuals_alive_at`.
This is because {func}`.individuals_alive_at` is a function that requires top-level metadata use,
which entails overhead that can be avoided by pre-extracting the metadata and passing it in.
Here this is not important because it is only run twice, but this would be essential
